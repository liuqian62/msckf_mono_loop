...
//  cout<<"====="<<endl;
  ...
  if (relo_msg != NULL)
  {
	  ...
	  u_v_id.x() = relo_msg->points[2*i].x;
	  u_v_id.y() = relo_msg->points[2*i].y;
	  u_v_id.z() = relo_msg->points[2*i].z;
...
void MsckfVio::addFeatureObservations(
  ...
  for (const auto& feature : msg->features) {
    if (map_server.find(feature.id) == map_server.end()) {
      ...
      map_server[feature.id].observations2[state_id] = Vector2d(feature.u0_o, feature.v0_o);
...
void MsckfVio::measurementJacobian(
    ...
    Matrix<double, 2, 6>& H_x, Matrix<double, 2, 3>& H_f, Vector2d& r) {  //CH 4 to 2
  ...
  Matrix<double, 2, 3> dz_dpc0 = Matrix<double, 2, 3>::Zero();  //CH 4 to 2
  ...
  Matrix<double, 2, 6> A = H_x;  // CH 4 to 2
  ...
  H_f = -H_x.block<2, 3>(0, 3);  // CH 4 to 2
  ...
  Vector2d tmpv = Vector2d(p_c0(0)/p_c0(2), p_c0(1)/p_c0(2)); // add
...
void MsckfVio::featureJacobian(
  ...
  jacobian_row_size = 2 * valid_cam_state_ids.size();  // CH 4 to 2
  ...
  for (const auto& cam_id : valid_cam_state_ids) {
    ...
    Matrix<double, 2, 6> H_xi = Matrix<double, 2, 6>::Zero();  // CH 4 to 2
    Matrix<double, 2, 3> H_fi = Matrix<double, 2, 3>::Zero();  // CH 4 to 2
    Vector2d r_i = Vector2d::Zero();  // CH 4 to 2
    ...
    // Stack the Jacobians.
    H_xj.block<2, 6>(stack_cntr, 21+6*cam_state_cntr) = H_xi;  // CH 4 to 2
    H_fj.block<2, 3>(stack_cntr, 0) = H_fi;  // CH 4 to 2
    r_j.segment<2>(stack_cntr) = r_i;  // CH 4 to 2
    stack_cntr += 2;  // CH 4 to 2
  }
...
void MsckfVio::removeLostFeatures() {
  ...
  for (auto iter = map_server.begin();
    ...
    jacobian_row_size += 2*feature.observations.size() - 3;  // CH 4 to 2
...
void MsckfVio::pruneCamStateBuffer() {
  ...
  for (auto& item : map_server) {
    ...
    jacobian_row_size += 2*involved_cam_state_ids.size() - 3;  // CH 4 to 2
  }
...
void MsckfVio::publish(const ros::Time& time) {
  ...
  for (const auto& item : map_server) {
    ...
    // ROS_INFO("u:%f:", feature.observations.find(state_server.imu_state.id)->second(0));
    p_2d.values.push_back(feature.observations2.find(state_server.imu_state.id)->second(0));
	  p_2d.values.push_back(feature.observations2.find(state_server.imu_state.id)->second(1));
    p_2d.values.push_back(feature.observations.find(state_server.imu_state.id)->second(0));
	  p_2d.values.push_back(feature.observations.find(state_server.imu_state.id)->second(1));
	  p_2d.values.push_back(feature.observations.find(state_server.imu_state.id)->second(0));
	  p_2d.values.push_back(feature.observations.find(state_server.imu_state.id)->second(1));
