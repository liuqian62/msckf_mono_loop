...
namespace msckf_vio
{
  ImageProcessor::ImageProcessor(ros::NodeHandle &n) : nh(n),
                                                       is_first_img(true),
                                                       //img_transport(n),
                                                       // stereo_sub(10),
  ...
  void ImageProcessor::cam0Callback(
 ...
 ros::Time orb_start = ros::Time::now();
    cv::Mat img1;
    cv::Mat img2;
    cv::Ptr<cv::CLAHE> clahe = cv::createCLAHE(3.0, cv::Size(8, 8));
    ...
    cv::Ptr<ORB> orb = ORB::create(2000);
    orb->setFastThreshold(0);
    //	ROS_INFO("!!!!!!!!!!!");
    orb->detectAndCompute(img1, Mat(), kp1, d1);
    orb->detectAndCompute(img2, Mat(), kp2, d2);

             ros::Time orb_end = ros::Time::now();
    ros::Duration orb_duration = orb_end - orb_start;
    ROS_INFO("orb_tracking time:%lf s", orb_duration.toSec());
    //	ROS_INFO("!!!!!!!!!!!");
    ...
    for (int i = 0; i < input_pts.size(); ++i)
    {
      ...
      for (int j = 0; j < matches_gms.size(); j++)
      {
        ...
        int x = input_pts[i].x - left.x;
        int y = input_pts[i].y - left.y;
        if (abs(x) + abs(y) < 15)
        {
  ...
  void ImageProcessor::trackFeatures()
  {
    ...
    ROS_INFO("tracking time:%lf s", duration.toSec());
    ...
    after_matching = curr_tracked_cam0_points.size();
  ...
  { //, Matx33f& cam1_R_p_c) {
    ...
    while (end_iter != imu_msg_buffer.end())
    {
      if ((end_iter->header.stamp -
           cam0_curr_img_ptr->header.stamp)
              .toSec() < 0.005)
        ++end_iter;
      else
        break;
